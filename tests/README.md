...
1. **`test_data_utils.py`**
   ...
   * Tests the core data processing components:
     ...
     * Data loaders: `FraudDataLoader`, `CreditCardDataLoader`, `IPCountryLoader`
...
4. **`test_modeling_pipeline.py`**

   * Tests the `ModelingPipeline` class:

     * Initialization
     * Data preparation
     * Logistic Regression and Random Forest training
     * Model evaluation, comparison, and selection
     * Ensures the pipeline runs without errors using sample synthetic data.
...
* Designed to catch regressions in data handling, feature engineering, preprocessing, and modeling.
* Warnings from underlying libraries (like `scikit-learn`) may appear but do not indicate test failure.
